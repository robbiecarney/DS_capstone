...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                {'label':'VAFB SLC-4E', 'value':'VAFB SLC-4E'}], value = 'ALL', placeholder = 'Select a site', searchable = True),
                                html.Br(),
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Pie chart of launch successes by site')
        return fig
    else:
        ...
        names='class', 
        title='Pie chart of successes and failures for site')
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site, range):
    spacex_df2 = spacex_df[(spacex_df['Payload Mass (kg)'] >= range[0]) & (spacex_df['Payload Mass (kg)'] <= range[1])]
    if entered_site == 'ALL':
        fig = px.scatter(spacex_df2, x = 'Payload Mass (kg)', y = 'class', color = 'Booster Version Category',
        title='Scatter chart of payload and success for all sites')
        return fig
    else:
        spacex_df3 = spacex_df2[spacex_df2['Launch Site']==entered_site]
        fig = px.scatter(spacex_df3, x = 'Payload Mass (kg)', y = 'class', color = 'Booster Version Category',
        title='Scatter chart of payload and success for selected site')
        return fig
